...
* @version $Id: pub_search.php,v 1.30 2010/01/06 20:27:15 orynider Exp $
 ...
 *
 */
class publisher_search extends publisher_public
{
	/**
	 * Enter description here...
	 *
	...
	{
		global $template, $lang, $board_config, $phpEx, $publisher_config, $db, $images;
		...
		$limit_sql = ( $start == 0 ) ? $publisher_config['pagination'] : $start . ',' . $publisher_config['pagination'];
		...
		if ( isset( $_POST['submit'] ) || $search_author != '' || $search_keywords != '' || $search_id )
		{
			...
			if ( $search_results != '' )
			{
				switch ( SQL_LAYER )
				{
					case 'oracle':
						$sql = "SELECT f1.*, AVG(r.rate_point) AS rating, COUNT(r.votes_file) AS total_votes, u.user_id, u.username, c.cat_id, c.cat_name, COUNT(cm.comments_id) AS total_comments
							FROM " . PUB_FILES_TABLE . " AS f1, " . PUB_VOTES_TABLE . " AS r, " . USERS_TABLE . " AS u, " . PUB_CATEGORY_TABLE . " AS c, " . PUB_COMMENTS_TABLE . " AS cm
					...
					default:
						$sql = "SELECT f1.*, AVG(r.rate_point) AS rating, COUNT(r.votes_file) AS total_votes, u.user_id, u.username, c.cat_id, c.cat_name, COUNT(cm.comments_id) AS total_comments
							FROM " . PUB_CATEGORY_TABLE . " AS c, " . PUB_FILES_TABLE . " AS f1
								LEFT JOIN " . PUB_VOTES_TABLE . " AS r ON f1.file_id = r.votes_file
								LEFT JOIN " . USERS_TABLE . " AS u ON f1.user_id = u.user_id
								LEFT JOIN " . PUB_COMMENTS_TABLE . " AS cm ON f1.file_id = cm.file_id
							WHERE f1.file_id IN ($search_results)
							AND c.cat_id = f1.file_catid
							AND f1.file_approved = '1'
							GROUP BY f1.file_id
				...
				for( $i = 0; $i < count( $searchset ); $i++ )
				{
					...
					// ===================================================
					$is_new = false;
					if ( time() - ( $publisher_config['settings_newdays'] * 24 * 60 * 60 ) < $searchset[$i]['file_time'] )
					{
						$is_new = true;
					}
					// ===================================================
					...
					{
						$posticon = '<img src="' . $images['pub_folder_sticky'] . '" border="0" />';
					}
				...
				$template->assign_vars( array(
					'PAGINATION' => phpBB2::generate_pagination( $base_url, $total_match_count, $publisher_config['pagination'], $start ),
					'PAGE_NUMBER' => sprintf( $lang['Page_of'], ( floor( $start / $publisher_config['pagination'] ) + 1 ), ceil( $total_match_count / $publisher_config['pagination'] ) ),
					'L_MODULE' => $publisher_config['module_name'],
				...
				$this->display( $lang['Download'], 'pub_search_result.tpl' );
			}
			else
			{
				mx_message_die( GENERAL_MESSAGE, $lang['No_search_match'] );
		...
		if ( !isset( $_POST['submit'] ) || ( $search_author == '' && $search_keywords == '' && !$search_id ) )
		{
			...
			$template->assign_vars( array(
				...
				'L_RATING' => $lang['Rated_downloads'],
				'L_PROJECTS' => $lang['Most_downloads'],
				'L_DATE' => $lang['Latest_downloads'],
				'L_NAME' => $lang['File_Title'],
			...
			$this->display( $lang['Download'], 'pub_search_body.tpl' );
		}
