...
* @version $Id: pub_download.php,v 1.25 2008/09/21 14:25:28 orynider Exp $
 ...
 *
 */
class publisher_download extends publisher_public
{
	/**
	 * Enter description here...
	 *
	...
	{
		global $lang, $db, $publisher_user, $publisher_config, $board_config, $phpEx, $userdata;
		global $phpbb_root_path, $publisher_functions;
		...
		if ( !MXBB_MODULE )
		{
			if ( !file_exists( $module_root_path . 'publisher/language/lang_' . $board_config['default_lang'] . '/lang_main.' . $phpEx ) )
			{
				include( $module_root_path . 'publisher/language/lang_english/lang_main.' . $phpEx );
			}
			else
			{
				include( $module_root_path . 'publisher/language/lang_' . $board_config['default_lang'] . '/lang_main.' . $phpEx );
			}
		}
		else
		{
		...
		if ( ( $publisher_config['hotlink_prevent'] ) and ( !empty( $url_referer ) ) )
		{
			...
			if ( $publisher_config['hotlink_allowed'] != '' )
			{
				$good_referers = explode( ',', $publisher_config['hotlink_allowed'] );
			}
			...
			if ( $errored )
			{
				mx_message_die( GENERAL_MESSAGE, $lang['Directly_linked'] );
			}
		}
		...
		if ( !empty( $mirrors_data ) && !$mirror_id )
		{
			...
			$template->assign_block_vars( 'mirror_row', array(
				'U_PROJECTS' => mx_append_sid( $$this->this_mxurl( 'action=download&file_id=' . $file_id . '&mirror_id=-1' ) ),
			...
			{
				$template->assign_block_vars( 'mirror_row', array(
					'U_PROJECTS' => mx_append_sid( $this->this_mxurl( 'action=download&file_id=' . $file_id . '&mirror_id=' . $mir_id ) ),
		...
		elseif ( ( !empty( $mirrors_data ) && $mirror_id == -1 ) || ( empty( $mirrors_data ) ) )
		{
			...
			$upload_dir = ( !empty( $file_data['upload_dir'] ) ) ? $file_data['upload_dir'] : $publisher_config['upload_dir'];
		...
		elseif ( $mirror_id > 0 && !empty( $mirrors_data[$mirror_id] ) )
		{
			...
			$upload_dir = ( !empty( $mirrors_data[$mirror_id]['upload_dir'] ) ) ? $mirrors_data[$mirror_id]['upload_dir'] : $publisher_config['upload_dir'];
		...
		// =========================================================================
		$publisher_user->update_info( $file_id );

		if ( !empty( $file_url ) )
		{
			$file_url = ( ( !strstr( $file_url, '://' ) ) && ( strpos( $file_url, 'publisher/uploads' ) === false ) ) ? 'http://' . $file_url : ( ( strpos( $file_url, 'publisher/uploads' ) && ( !strstr( $file_url, '://' ) ) ) ? $module_root_path . $file_url : $file_url );
			pub_redirect( $file_url );
		}
		else
		{
