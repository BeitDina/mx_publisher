...
* @version $Id: pub_stats.php,v 1.28 2011/12/29 05:51:52 orynider Exp $
 ...
 *
 */
class publisher_stats extends publisher_public
{
	/**
	 * Enter description here...
	 *
	...
	function main( $action  = false )
	{
		...
		$sql = "SELECT r.votes_file, AVG(r.rate_point) AS rating, f.file_id, f.file_name
			FROM " . PUB_VOTES_TABLE . " AS r, " . PUB_FILES_TABLE . " AS f
		...
		if (@file_exists($module_root_path . "publisher/images/os/"))
		{
			$osEx = ".png";
			$osDir = "os/";
		}
		...
		if (@file_exists($module_root_path . "publisher/images/browsers/"))
		{
			$agentEx = ".png";
			$agentDir = "browsers/";
		}
		...
		$os_graphic = 0;
		$os_graphic_max = count( $images['pub_voting_graphic'] );

		foreach( $os_point as $index => $point )
		{
			$temp_point = ( $point > 100 ) ? 100 : $point;
			$os_graphic_img = $images['pub_voting_graphic'][$os_graphic];
			$os_graphic = ( $os_graphic < $os_graphic_max - 1 ) ? $os_graphic + 1 : 0;

			$template->assign_block_vars("downloads_os", array('OS_IMG' => $module_root_path . "publisher/images/" . $osDir . $os_image[$index],
		...
		$b_graphic = 0;
		$b_graphic_max = count( $images['pub_voting_graphic'] );

		foreach( $agent_point as $index => $point )
		{
			$temp_point = ( $point > 100 ) ? 100 : $point;
			$b_graphic_img = $images['pub_voting_graphic'][$b_graphic];
			$b_graphic = ( $b_graphic < $b_graphic_max - 1 ) ? $b_graphic + 1 : 0;

			$template->assign_block_vars( "downloads_b", array(
				'B_IMG' => $module_root_path . "publisher/images/" . $agentDir . $agent_image[$index],
		...
		foreach( $os_point as $index => $point )
		{
			...
			$template->assign_block_vars( "rating_os", array(
				'OS_IMG' => $module_root_path . "publisher/images/" . $osDir . $os_image[$index],
				'OS_NAME' => $os_lang[$index],
				'OS_OPTION_RESULT' => $point,
				'OS_OPTION_IMG' => $os_graphic_img,
		...
		$this->display( $lang['Download'], 'pub_stats_body.tpl' );
	}
}
?>