...
* @version $Id: pub_user_upload.php,v 1.33 2009/07/29 05:08:13 orynider Exp $
 ...
 *
 */
class publisher_user_upload extends publisher_public
{
	/**
	 * Enter description here...
	 *
	...
	function main( $action  = false )
	{
		...
		if ( isset( $_POST['submit'] ) && $cat_id )
		{
			...
			// Notification
			//
			$this->update_add_item_notify($file_id, $pub_post_mode);
			...
			if ( $this->comments[$cat_id]['activated'] && $this->comments[$cat_id]['autogenerate_comments'] )
			{
				...
				$mx_pub_notification->init( $file_id, $publisher_config['allow_comment_wysiwyg'] );
				$mx_pub_notification->_compose_auto_note($pub_post_mode == 'add' ? MX_NEW_NOTIFICATION : MX_EDITED_NOTIFICATION);

				//
				// Generate comment
				//
				$this->update_add_comment('', $file_id, 0, addslashes(trim($mx_pub_notification->topic_title)), addslashes(trim($mx_pub_notification->message)), true, false, false, true);
			}
		...
		// =======================================================
		{
			...
			$this->display( $lang['Download'], 'pub_file_add.tpl' );
		}
	}
}
?>