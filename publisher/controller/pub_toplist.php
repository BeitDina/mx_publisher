...
* @version $Id: pub_toplist.php,v 1.29 2010/01/06 20:27:15 orynider Exp $
 ...
 *
 */
class publisher_toplist extends publisher_public
{
	/**
	 * Enter description here...
	 *
	...
	function main( $action  = false )
	{
		...
		switch ( $mode )
		{
				...
				else
				{
					...
					for ( $i = 0; $i < count( $file_rowset ); $i++ )
					{
						...
						$template->assign_block_vars( 'files_row', array( 'CAT_NAME' => $cat_name,
							...
							'PROJECTS' => $file_rowset[$i]['file_dls'],
			...
			case 'rating':
				...
				$join_statement = ( $mode == 'rating' ) ? 'LEFT JOIN ' . PUB_VOTES_TABLE . ' AS r ON f.file_id = r.votes_file' : '';
					...
					switch ( SQL_LAYER )
					{
						case 'oracle':
							$sql = "SELECT f1.*, AVG(r.rate_point) AS rating, COUNT(r.votes_file) AS total_votes, u.user_id, u.username, c.cat_id, c.cat_name, c.cat_name, c.cat_allow_ratings, c.cat_allow_comments
								FROM " . PUB_FILES_TABLE . " AS f1, " . PUB_VOTES_TABLE . " AS r, " . USERS_TABLE . " AS u, " . PUB_CATEGORY_TABLE . " AS c
								WHERE f1.file_id = r.votes_file(+)
								AND f1.user_id = u.user_id(+)
								AND c.cat_id = f1.file_catid
								AND f1.file_approved = '1'
				...
				for( $i = 0; $i < count( $searchset ); $i++ )
				{
					...
					$template->assign_block_vars( 'files_row', array(
						...
						'DATE' => $date,
						'UPDATED' => $date_updated,
						'RATING' => ( $searchset[$i]['cat_allow_ratings'] ? $rating : $lang['kb_no_ratings'] ),
						'SHOW_RATINGS' => ( $pub_use_ratings ?  true : false ),
		...
		$template->assign_vars( array( 'SHOW_RATINGS' => ( $pub_use_ratings ) ? true : false ) );

		// ===================================================
		// assign var for navigation
		// ===================================================

		$this->display( $lang['Download'], 'pub_toplist_body.tpl' );
	}
}
