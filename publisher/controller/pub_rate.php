...
* @version $Id: pub_rate.php,v 1.22 2008/09/21 14:25:36 orynider Exp $
 ...
 *
 */
class publisher_rate extends publisher_public
{
	/**
	 * Enter description here...
	 *
	...
	{
		global $template, $lang, $board_config, $phpEx, $publisher_config, $db, $userdata;
		global $phpbb_root_path, $publisher_user, $publisher_functions;
		...
		if ( ( !$this->auth_user[$file_data['file_catid']]['auth_rate'] ) )
		{
			...
			$message = sprintf( $lang['Sorry_auth_rate'], $this->auth_user[$file_data['file_catid']]['auth_rate_type'] );
		...
		if ( isset( $_POST['submit'] ) )
		{
			...
			$rate_info = $publisher_functions->get_rating( $file_id );

			$result_msg = str_replace( "{newrating}", $rate_info, $result_msg );

			$message = $result_msg . '<br /><br />' . sprintf( $lang['Click_return'], '<a href="' . mx_append_sid( $this->this_mxurl( 'action=file&file_id=' . $file_id ) ) . '">', '</a>' );
			mx_message_die( GENERAL_MESSAGE, $message );
		}
		else
		{
		...
		$this->display( $lang['Download'], 'pub_rate_body.tpl' );
	}
}
?>