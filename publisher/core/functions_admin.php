...
* @version $Id: functions_admin.php,v 1.10 2008/09/24 12:13:41 orynider Exp $
...
/**
 * Public publisher_admin class.
 *
 */
class publisher_admin extends publisher_public
{
	/**
	 * load admin module
	 *
	...
	function adminmodule( $module_name )
	{
		if ( !class_exists( 'publisher_' . $module_name ) )
		{
			...
			require_once( $module_root_path . 'publisher/admin/admin_' . $module_name . '.' . $phpEx );
	...
	{
		global $lang, $phpbb_root_path, $template, $phpEx;
		...
		if ( isset( $this->subcat_rowset[$cat_parent] ) )
		{
			foreach( $this->subcat_rowset[$cat_parent] as $sub_cat_id => $cat_data )
			{
				$template->assign_block_vars( 'cat_row', array(
					...
					'PRE' => $pre,
					'U_CAT' => mx_append_sid( "admin_publisher.$phpEx?action=catauth_manage&cat_parent=$sub_cat_id" )
				));
		...
		if ( isset( $this->subcat_rowset[$cat_parent] ) )
		{
			foreach( $this->subcat_rowset[$cat_parent] as $sub_cat_id => $cat_data )
			{
				$template->assign_block_vars( 'cat_row', array(
					...
					'IS_HIGHER_CAT' => ( $cat_data['cat_allow_file'] ) ? false : true,
					'PRE' => $pre,
					'U_CAT' => mx_append_sid( "admin_publisher.$phpEx?action=catauth&cat_id=$sub_cat_id" ),
	...
	{
		global $phpbb_root_path, $template, $phpEx, $lang;
		...
		if ( isset( $this->subcat_rowset[$cat_parent] ) )
		{
			...
			{
				$template->assign_block_vars( 'cat_row', array(
					'IS_HIGHER_CAT' => ( $cat_data['cat_allow_file'] == PUB_CAT_ALLOW_FILE ) ? false : true,
					'U_CAT' => mx_append_sid( "admin_publisher.$phpEx?action=cat_manage&cat_id=$subcat_id" ),
					'U_CAT_EDIT' => mx_append_sid( "admin_publisher.$phpEx?action=cat_manage&mode=edit&amp;cat_id=$subcat_id" ),
					'U_CAT_DELETE' => mx_append_sid( "admin_publisher.$phpEx?action=cat_manage&mode=delete&amp;cat_id=$subcat_id" ),
					'U_CAT_MOVE_UP' => mx_append_sid( "admin_publisher.$phpEx?action=cat_manage&mode=cat_order&amp;move=-15&amp;cat_id_other=$subcat_id" ),
					'U_CAT_MOVE_DOWN' => mx_append_sid( "admin_publisher.$phpEx?action=cat_manage&mode=cat_order&amp;move=15&amp;cat_id_other=$subcat_id" ),
					'U_CAT_RESYNC' => mx_append_sid( "admin_publisher.$phpEx?action=cat_manage&mode=sync&amp;cat_id_other=$subcat_id" ),
					'CAT_NAME' => isset($lang[$cat_data['cat_name']]) ? $lang[$cat_data['cat_name']] : $cat_data['cat_name'],
					'PRE' => $pre
				));
	...
	function pub_size_select( $select_name, $size_compare )
	{
		global $lang;
	...
	function update_add_cat( $cat_id = false )
	{
		...
		if ( !$cat_id )
		{
			...
			$sql = "INSERT INTO " . PUB_CATEGORY_TABLE . " (cat_name, cat_desc, cat_parent, parents_data, cat_order, cat_allow_file, cat_allow_ratings, cat_allow_comments, internal_comments, autogenerate_comments, comments_forum_id, show_pretext, notify, notify_group)
	...
	function delete_cat( $cat_id = false )
	{
		...
		else
		{
			...
			$sql = 'DELETE FROM ' . PUB_CATEGORY_TABLE . "
				WHERE cat_id = $cat_id";
