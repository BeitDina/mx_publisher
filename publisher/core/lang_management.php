...
class lang_management extends publisher_admin
{
	...
	function get_countries()
	{
		...
		$countries = array();
		$dir = @opendir($module_root_path . 'language');
		while ($file = @readdir($dir))
		{
			if (preg_match('#^lang_#i', $file) && !is_file($module_root_path . 'language/' . $file) && !is_link($module_root_path . 'language/' . $file))
			{
	...
	function get_packs()
	{
		...
		/* MG Lang DB - BEGIN */
		$skip_files = array(('lang_bbcode.' . $phpEx), ('lang_faq.' . $phpEx), ('lang_rules.' . $phpEx));
		/* MG Lang DB - END */
		...
		{
			$dir = @opendir($module_root_path . 'language/' . $country_dir);
			while ($file = readdir($dir))
			{
				$pattern = 'lang_u';
				...
				if (preg_match('/' . $pattern . '/i', $file))
				{
					$displayname = trim(str_replace(('.' . $phpEx), '', str_replace('lang_', '', $file)));
					$packs[$file] = $displayname;
				}
				/* Ory Lang DB - END */
			}
	...
	{
		global $countries, $packs, $module_root_path, $phpEx;

		// get filename
		$file = $module_root_path . 'language/' . $country_dir . '/' . $pack_file;
		...
		for ($i = 0; $i < 2; $i++)
		{
			...
			if ($pack_file == 'lang')
			{
				$file = $module_root_path . 'language/' . $country_dir . '/' . ($lang_extend_admin ? 'lang_admin.' : 'lang_main.') . $phpEx;
			}
			// fix the filename for custom keys
			if ($pack_file == 'custom')
			{
				$file = $module_root_path . 'language/' . $country_dir . '/' . 'lang_extend.' . $phpEx;
			}
			*/
			/* MG Lang DB - END */
	...
	function get_entries($modified = true)
	{
		...
		// init
		$entries = array();
		$countries = isset($countries) ? $countries : $this->get_countries();
		$packs = isset($packs) ? $packs : $this->get_packs($countries);
	...
	function write($entries)
	{
		...
		global $countries, $packs, $module_root_path;
		...
		while (list($country_dir, $country_name) = each($countries))
		{
			...
			$array_replace = array(
				'$' . 'Id: ' . $pack_filename . ' ' . $edittime . ' ' . $mx_user->data['username'] . ' $',
				$country_name,
				$edittime,
				$mx_user->data['username'],
			);

			$file_content = str_replace($array_find, $array_replace, $file_content);

			$filename = $module_root_path . 'language/' . $country_dir . '/' . $pack_filename;
