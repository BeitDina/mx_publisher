<?php
/**
*
* @package MX-Publisher Module - mx_publisher
* @version $Id: lang_cat_extend.php,v 1.9 2008/07/15 22:07:04 admin $
* @copyright (c) 2002-2006 [Jon Ohlsson, Mohd Basri, wGEric, PHP Arena, FlorinCB, CRLin] MX-Publisher Project Team
* @license http://opensource.org/licenses/gpl-license.php GNU General Public License v2
*
*/

/**
*
* @Extra credits for this file
* FlorinCB (orynider@beitdina.net)
*
* Added backends and equal in Notepad++
* expr: ^([\d ]+[\d]+)[ ]*$
* word regex: ^(\w+)
* repl: $lang['\1'] =
*/

/**
*
* [English]
* LAST UPDATE 2018-10-16 08:56:11 by admin
*
*/

$lang['MY_CATEGORY'] = 'Categoria Mea';
?>