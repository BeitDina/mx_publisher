...
* @version $Id: publisher_common.php,v 1.28 2009/07/29 05:08:13 orynider Exp $
...
// Include Files
// ===================================================
include( $module_root_path . 'publisher/core/publisher_constants.' . $phpEx );

//
// Load addon tools
//
// - Class module_cache
// - Class mx_custom_fields (publisher needs its own class version in functions.php)
...
if ( !MXBB_MODULE )
{
	...
	@include($phpbb_root_path . $template_path . $template_name . '/' . 'publisher.cfg');
...
include_once($module_root_path . 'publisher/core/publisher_functions.' . $phpEx);
include_once($module_root_path . 'publisher/core/mx_publisher_auth.' . $phpEx );
include_once($module_root_path . 'publisher/core/mx_publisher.' . $phpEx);
include_once($module_root_path . 'publisher/core/publisher_cache.' . $phpEx); //Temp fix
...
if ( !MXBB_MODULE )
{
	include_once($module_root_path . 'publisher/core/template.' . $phpEx); // Include XS template
...
// Load classes
// ===================================================
$publisher_cache = new publisher_cache($module_root_path . 'publisher/');
$publisher_functions = new publisher_functions();

$publisher_config = $publisher_functions->obtain_publisher_config();
		
if ($publisher_config['allow_comment_wysiwyg'] == '')
{
	$publisher_config = $publisher_functions->obtain_publisher_config(false);
}

$publisher_user = new mx_user_info();
...
{
	$publisher = new publisher_public();
}
?>