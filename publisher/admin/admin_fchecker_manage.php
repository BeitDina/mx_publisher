...
class publisher_fchecker_manage extends publisher_admin
{
	function main( $module_id = false )
	{
		...
		$this_dir = $module_root_path . 'publisher/uploads/';

		// MX
		$html_path = PORTAL_URL . $module_root_path . 'publisher/uploads/';
		...
		$template->set_filenames( array( 'admin' => 'admin/pub_admin_file_checker.tpl' ) );
		...
		if ( $safety == 1 )
		{
			...
			$sql = "SELECT * FROM " . PUB_FILES_TABLE;
			...
			while ( $temp = $db->sql_fetchrow( $overall_result ) )
			{
				...
				{
					/*$sql = "UPDATE " . PUB_FILES_TABLE . " SET file_ssurl='' WHERE file_id = '" . $temp_file_id . "'";
			...
			while ( $temp = readdir( $files ) )
			{
				...
				$sql = "SELECT * FROM " . PUB_FILES_TABLE . " WHERE file_dlurl = '" . $html_path . $temp . "' OR file_ssurl = '" . $html_path . $temp . "'";
