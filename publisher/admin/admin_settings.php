...
class publisher_settings extends publisher_admin
{
	function main( $module_id = false )
	{
		$action = $module_id;
		global $db, $images, $template, $lang, $phpEx, $publisher_functions, $publisher_cache, $publisher_config, $phpbb_root_path, $module_root_path, $mx_root_path, $mx_request_vars, $portal_config;
		...
		else
		{
			while ( $row = $db->sql_fetchrow( $result ) )
			{
				...
				$new[$config_name] = ( isset( $_POST[$config_name] ) ) ? $_POST[$config_name] : $pub_config[$config_name];

				if ( ( empty( $size ) ) && ( !$submit ) && ( $config_name == 'max_file_size' ) )
				{
					$size = ( intval( $pub_config[$config_name] ) >= 1048576 ) ? 'mb' : ( ( intval( $pub_config[$config_name] ) >= 1024 ) ? 'kb' : 'b' );
				}
				...
				if ( $submit )
				{
					...
					if ( $config_name == 'tpl_php' && isset( $_POST[$config_name] ) && $new[$config_name] != $pub_config[$config_name] )
					{
						$template->compile_cache_clear();
					}

					$publisher_functions->set_config( $config_name, $new[$config_name] );
				}
			}
			...
			if ( $submit )
			{
				...
				$message = $lang['Settings_changed'] . '<br /><br />' . sprintf( $lang['Click_return'], '<a href="' . mx_append_sid( "admin_publisher.$phpEx?action=settings" ) . '">', '</a>' ) . '<br /><br />' . sprintf( $lang['Click_return_admin_index'], '<a href="' . mx_append_sid( $mx_root_path . "admin/index.$phpEx?pane=right" ) . '">', '</a>' );
				mx_message_die( GENERAL_MESSAGE, $message );
			}
		}

		$template->set_filenames( array( 'admin' => 'admin/pub_admin_settings.tpl' ) );
		...
		$template->assign_vars( array(
			'S_SETTINGS_ACTION' => mx_append_sid( "admin_publisher.$phpEx?action=settings" ),
			...
			'S_FILESIZE' => $this->pub_size_select( 'max_size', $size ),
