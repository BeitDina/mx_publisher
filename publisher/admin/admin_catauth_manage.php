...
class publisher_catauth_manage extends publisher_admin
{
	function main($module_id = false)
	{
		...
		$cat_auth_fields = array('auth_view', 'auth_read', 'auth_view_file', 'auth_edit_file', 'auth_delete_file', 'auth_post', 'auth_upload', 'auth_download', 'auth_rate', 'auth_email', 'auth_view_comment', 'auth_post_comment', 'auth_edit_comment', 'auth_delete_comment');
		...
		if (isset($_POST['submit']))
		{
			...
			$message = $lang['Category_auth_updated'] . '<br /><br />' . sprintf($lang['Click_return_catauth'], '<a href="' . mx_append_sid("admin_publisher.$phpEx?action=catauth_manage") . '">', "</a>");
		...
		// specified
		$template->set_filenames(array('body' => 'admin/pub_auth_cat_body.tpl'));
		...
		elseif (!empty($cat_id))
		{
			$template->assign_block_vars('cat_row', array(
				...
				'U_CAT' => mx_append_sid("admin_publisher.$phpEx?action=catauth_manage&cat_parent={$this->cat_rowset[$cat_id]['cat_parent']}")
			));
		...
		$template->assign_vars( array(
			...
			'S_CATAUTH_ACTION' => mx_append_sid("admin_publisher.$phpEx?action=catauth_manage"),
