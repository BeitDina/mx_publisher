...
class publisher_ug_auth_manage extends publisher_admin
{
	function main( $module_id = false )
	{
		...
		$permissions_menu = array(
			mx_append_sid( "admin_publisher.$phpEx?action=catauth_manage" ) => $lang['Cat_Permissions'],
			mx_append_sid( "admin_publisher.$phpEx?action=ug_auth_manage&mode=user" ) => $lang['User_Permissions'],
			mx_append_sid( "admin_publisher.$phpEx?action=ug_auth_manage&mode=group" ) => $lang['Group_Permissions'],
			mx_append_sid( "admin_publisher.$phpEx?action=ug_auth_manage&mode=global_user" ) => $lang['User_Global_Permissions'],
			mx_append_sid( "admin_publisher.$phpEx?action=ug_auth_manage&mode=global_group" ) => $lang['Group_Global_Permissions']
		);
		...
		if ( isset( $_POST['submit'] ) && ( ( $mode == 'user' && $user_id ) || ( $mode == 'group' && $group_id ) ) )
		{
			...
			switch (PORTAL_BACKEND)
			{
				...
				case 'phpbb2':
					...
					$sql = ( $mode == 'user' ) ? "SELECT aa.* FROM " . PUB_AUTH_ACCESS_TABLE . " aa, " . USER_GROUP_TABLE . " ug, " . GROUPS_TABLE . " g WHERE ug.user_id = $user_id AND g.group_id = ug.group_id AND aa.group_id = ug.group_id AND g.group_single_user = " . true : "SELECT * FROM " . PUB_AUTH_ACCESS_TABLE . " WHERE group_id = $group_id";					
							
				break;

				case 'phpbb3':
					
					$sql = ( $mode == 'user' ) ? "SELECT aa.* FROM " . PUB_AUTH_ACCESS_TABLE . " aa, " . USER_GROUP_TABLE . " ug, " . GROUPS_TABLE . " g LEFT JOIN " . USERS_TABLE . " u ON (ug.group_id = u.group_id) WHERE ug.user_id = $user_id AND g.group_id = ug.group_id AND aa.group_id = ug.group_id" : "SELECT * FROM " . PUB_AUTH_ACCESS_TABLE . " WHERE group_id = $group_id";						
			...
			while ( list( $cat_id, $action ) = @each( $cat_auth_action ) )
			{
				...
				else
				{
					if ( $action == 'insert' )
					{
						...
						$sql = "INSERT INTO " . PUB_AUTH_ACCESS_TABLE . " (cat_id, group_id, $sql_field)
					...
					else
					{
						...
						$sql = "UPDATE " . PUB_AUTH_ACCESS_TABLE . "
							SET $sql_values
			...
			if ( $delete_sql != '' )
			{
				$sql = "DELETE FROM " . PUB_AUTH_ACCESS_TABLE . "
					WHERE group_id = $group_id
			...
			$message = $lang['Auth_updated'] . '<br /><br />' . sprintf( $l_auth_return, '<a href="' . mx_append_sid( "admin_publisher.$phpEx?action=ug_auth_manage&mode=$mode" ) . '">', '</a>' ) . '<br /><br />' . sprintf( $lang['Click_return_admin_index'], '<a href="' . mx_append_sid( $mx_root_path . "admin/index.$phpEx?pane=right" ) . '">', '</a>' );
		...
		elseif ( isset( $_POST['submit'] ) && ( ( $mode == 'global_user' && $user_id ) || ( $mode == 'global_group' && $group_id ) ) )
		{
			...
			$sql = ( $mode == 'global_user' ) ? "SELECT aa.* FROM " . PUB_AUTH_ACCESS_TABLE . " aa, " . USER_GROUP_TABLE . " ug, " . GROUPS_TABLE . " g WHERE ug.user_id = $user_id AND g.group_id = ug.group_id AND aa.group_id = ug.group_id AND g.group_single_user = " . true . " AND aa.cat_id = '0'" : "SELECT * FROM " . PUB_AUTH_ACCESS_TABLE . " WHERE group_id = $group_id AND cat_id = '0'";
			if ( !( $result = $db->sql_query( $sql ) ) )
			{
				mx_message_die( GENERAL_ERROR, "Couldn't obtain user/group permissions", "", __LINE__, __FILE__, $sql );
			}
			...
			for( $j = 0; $j < count( $global_auth_fields ); $j++ )
			{
				...
				if ( $publisher_config[$auth_field] == AUTH_ACL && isset( $change_acl_list[$auth_field] ) )
				{
		...
		elseif ( ( $mode == 'user' && ( isset( $_POST['username'] ) || $user_id ) ) || ( $mode == 'group' && $group_id ) )
		{
			...
			switch (PORTAL_BACKEND)
			{
				...
				case 'phpbb2':
					...
					$sql = ( $mode == 'user' ) ? "SELECT aa.*, g.group_single_user FROM " . PUB_AUTH_ACCESS_TABLE . " aa, " . USER_GROUP_TABLE . " ug, " . GROUPS_TABLE . " g WHERE ug.user_id = $user_id AND g.group_id = ug.group_id AND aa.group_id = ug.group_id AND g.group_single_user = 1" : "SELECT * FROM " . PUB_AUTH_ACCESS_TABLE . " WHERE group_id = $group_id";					
				break;

				case 'phpbb3':
				
					$sql_user = 'SELECT aa.*, g.group_name, g.group_id, g.group_type
						FROM ' . GROUPS_TABLE . ' g, ' . PUB_AUTH_ACCESS_TABLE . ' aa
							LEFT JOIN ' . USER_GROUP_TABLE . ' ug ON (ug.group_id = g.group_id)
						WHERE ug.user_id = ' . $user_id . '
			...
			if ( count( $name ) )
			{
				...
				for( $i = 0; $i < count( $ug_info ); $i++ )
				{
					...
					$t_usergroup_list .= ( ( $t_usergroup_list != '' ) ? ', ' : '' ) . '<a href="' . mx_append_sid( "admin_publisher.$phpEx?action=ug_auth_manage&mode=$ug=" . $id[$i] ) . '">' . $name[$i] . '</a>';
				}
			}
			else
			{
			...
			$template->assign_vars( array(
				...
				'U_USER_OR_GROUP' => mx_append_sid( "admin_publisher.$phpEx?action=ug_auth_manage" ),

				'S_COLUMN_SPAN' => $s_column_span + 2,
				'S_AUTH_ACTION' => mx_append_sid( "admin_publisher.$phpEx?action=ug_auth_manage" ),
		...
		elseif ( ( $mode == 'global_user' && ( isset( $_POST['username'] ) || $user_id ) ) || ( $mode == 'global_group' && $group_id ) )
		{
			...
			for( $k = 0; $k < count( $global_auth_fields ); $k++ )
			{
				...
				if ( $publisher_config[$field_name] == AUTH_ACL )
				{
			...
			$template->assign_block_vars( 'cat_row', array(
				...
				'U_CAT' => mx_append_sid( "admin_publisher.$phpEx?action=settings" )
			));
		...
		else
		{
			...
			$template->assign_vars( array(
				...
				'S_HIDDEN_FIELDS' => $s_hidden_fields,
				'S_' . $l_type . '_ACTION' => mx_append_sid( "admin_publisher.$phpEx?action=ug_auth_manage" )
			));
		}
