...
class publisher_custom_manage extends publisher_admin
{
	function main( $module_id = false )
	{
		...
		{
			case 'addfield':
				$template_file = 'admin/pub_admin_field_add.tpl';
				break;
			case 'editfield':
				$template_file = 'admin/pub_admin_field_add.tpl';
				break;
			case 'edit':
				$template_file = 'admin/pub_admin_select_field.tpl';
				break;
			case 'add':
				$template_file = 'admin/pub_admin_select_field_type.tpl';
				break;
			case 'delete':
				$template_file = 'admin/pub_admin_field_delete.tpl';
				break;
			case 'select':
				$template_file = 'admin/pub_admin_field.tpl';
				break;
		}
		...
		if ( $submit )
		{
			if ( $mode == 'do_add' && !$field_id )
			{
				...
				$message = $lang['Fieldadded'] . '<br /><br />' . sprintf( $lang['Click_return'], '<a href="' . mx_append_sid( 'admin_publisher.' . $phpEx . '?action=custom_manage' ) . '">', '</a>' ) . '<br /><br />' . sprintf( $lang['Click_return_admin_index'], '<a href="' . mx_append_sid( 'index.' . $phpEx . '?pane=right' ) . '">', '</a>' );
			...
			elseif ( $mode == 'do_add' && $field_id )
			{
				...
				$message = $lang['Fieldedited'] . '<br /><br />' . sprintf( $lang['Click_return'], '<a href="' . mx_append_sid( 'admin_publisher.' . $phpEx . '?action=custom_manage' ) . '">', '</a>' ) . '<br /><br />' . sprintf( $lang['Click_return_admin_index'], '<a href="' . mx_append_sid( 'index.' . $phpEx . '?pane=right' ) . '">', '</a>' );
				mx_message_die( GENERAL_MESSAGE, $message );
			}
			elseif ( $mode == 'do_delete' )
			{
				...
				$message = $lang['Fieldsdel'] . '<br /><br />' . sprintf( $lang['Click_return'], '<a href="' . mx_append_sid( 'admin_publisher.' . $phpEx . '?action=custom_manage' ) . '">', '</a>' ) . '<br /><br />' . sprintf( $lang['Click_return_admin_index'], '<a href="' . mx_append_sid( 'index.' . $phpEx . '?pane=right' ) . '">', '</a>' );
				mx_message_die( GENERAL_MESSAGE, $message );
			}
		}
		...
		$template->assign_vars( array(
			...
			'S_FIELD_ACTION' => mx_append_sid( "admin_publisher.$phpEx?action=custom_manage" )
		));
