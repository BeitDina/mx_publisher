...
class publisher_cat_manage extends publisher_admin
{
	function main( $module_id = false )
	{
		$action = $module_id;
		global $db, $images, $template, $template, $lang, $phpEx, $publisher_functions, $publisher_cache, $publisher_config, $phpbb_root_path, $module_root_path, $mx_root_path, $mx_request_vars, $portal_config;
		...
		if ( $mode == 'do_add' && !$cat_id )
		{
			...
			{
				$this->_publisher();
				$message = $lang['Catadded'] . '<br /><br />' . sprintf( $lang['Click_return'], '<a href="' . mx_append_sid( "admin_publisher.$phpEx?action=cat_manage" ) . '">', '</a>' ) . '<br /><br />' . sprintf( $lang['Click_edit_permissions'], '<a href="' . mx_append_sid( "admin_publisher.$phpEx?action=catauth_manage&cat_id=$new_cat_id" ) . '">', '</a>' );
		...
		elseif ( $mode == 'do_add' && $cat_id )
		{
			...
			{
				$this->_publisher();
				$message = $lang['Catedited'] . '<br /><br />' . sprintf( $lang['Click_return'], '<a href="' . mx_append_sid( "admin_publisher.$phpEx?action=cat_manage" ) . '">', '</a>' ) . '<br /><br />' . sprintf( $lang['Click_edit_permissions'], '<a href="' . mx_append_sid( "admin_publisher.$phpEx?action=catauth_manage&cat_id=$new_cat_id" ) . '">', '</a>' );
				mx_message_die( GENERAL_MESSAGE, $message );
			}
			$mode = 'edit';
		}
		elseif ( $mode == 'do_delete' )
		{
			...
			{
				$this->_publisher();
				$message = $lang['Catsdeleted'] . '<br /><br />' . sprintf( $lang['Click_return'], '<a href="' . mx_append_sid( "admin_publisher.$phpEx?action=cat_manage" ) . '">', '</a>' ) . '<br /><br />' . sprintf( $lang['Click_return_admin_index'], '<a href="' . mx_append_sid( $mx_root_path . "admin/index.$phpEx?pane=right" ) . '">', '</a>' );
				mx_message_die( GENERAL_MESSAGE, $message );
			}
		}
		...
		switch ( $mode )
		{
			...
			case 'edit':
				...
				break;
			case 'delete':
				$template_file = 'admin/pub_admin_cat_delete.tpl';
		...
		if ( sizeof( $this->error ) ) $template->assign_block_vars( 'publisher_error', array() );
		...
		$template->assign_vars( array(
			...
			'S_CAT_ACTION' => mx_append_sid( "admin_publisher.$phpEx?action=cat_manage" )
		));
