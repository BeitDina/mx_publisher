...
class publisher_lang_user_created extends publisher_admin
{
	function main( $module_id = false )
	{
		$action = $module_id;
		global $mx_user, $db, $images, $template, $lang, $phpEx, $publisher_functions, $publisher_cache, $board_config, $publisher_config, $phpbb_root_path, $module_root_path, $mx_root_path, $mx_request_vars, $portal_config;
		...
		if (!include($module_root_path . 'publisher/core/lang_management.' . $phpEx ))
		{
			die('Cant find ' . $module_root_path . 'publisher/core/lang_management.' . $phpEx);
		}
		...
		// get packs installed
		$packs = $lang_management->get_packs($countries);
		...
		if ($mode == 'key')
		{
			if ($delete)
			{
				...
				// send message
				$pack_url = mx_append_sid('admin_publisher.' . $phpEx . '?action=lang_user_created&amp;mode=pack&amp;pack=' . urlencode($pack_file) . '&amp;level=' . (($level == 'normal') ? 'normal' : 'admin'));
			...
			elseif ($submit)
			{
				...
				// send message
				$key_url = mx_append_sid('admin_publisher.' . $phpEx . '?action=lang_user_created&amp;mode=key&amp;pack=' . urlencode($new_pack) . '&amp;key=' . urlencode($new_main) . '&amp;sub=' . urlencode($new_sub) . '&amp;level=' . urlencode(($new_level == 'normal') ? 'normal' : 'admin'));
				$pack_url = mx_append_sid('admin_publisher.' . $phpEx . '?action=lang_user_created&amp;mode=pack&amp;pack=' . urlencode($new_pack) . '&amp;level=' . (($new_level == 'normal') ? 'normal' : 'admin'));
			...
			else
			{
				...
				$key_main = $key_main ? $key_main : '';
				$key_sub = $key_sub ? $key_sub : '';
				...
				$template->assign_vars(array(
					...
					'L_PACKS_EXPLAIN'			=> $lang['Lang_extend_pack_explain'],
					)
				);
				...
				$template->assign_vars(array(
					'S_ACTION'				=> mx_append_sid('admin_publisher.'.$phpEx.'?action=lang_user_created'),
		...
		if ($mode == 'pack')
		{
			...
			else
			{
				...
				$template->assign_vars(array(
					...
					'L_PACK'					=> $lang['Lang_extend_pack'],
					'U_PACK'					=> mx_append_sid('admin_publisher.' . $phpEx . '?action=lang_user_created'),
					...
					'U_LEVEL_NEXT'		=> mx_append_sid('admin_publisher.' . $phpEx . '?action=lang_user_created&amp;mode=pack&amp;pack=' . urlencode($pack_file) . '&amp;level=' . (($level == 'admin') ? 'normal' : 'admin')),
					...
					while (list($key_sub, $pack) = @each($data))
					{
						...
						if (($pack == $pack_file) && (($entries['admin'][$key_main][$key_sub] && ($level == 'admin')) || (!$entries['admin'][$key_main][$key_sub] && ($level == 'normal'))))
						{
							...
							$template->assign_block_vars('row', array(
								...
								'U_KEY'			=> mx_append_sid('admin_publisher.' . $phpEx . '?action=lang_user_created&amp;mode=key&amp;pack=' . urlencode($pack_file) . '&amp;level=' . $level . '&amp;key=' . urlencode($key_main) . '&amp;sub=' . urlencode($key_sub)),
								'VALUE'			=> $value,
		...
		if ($mode == 'search')
		{
			...
			else
			{
				...
				{
					$main_url = mx_append_sid('admin_publisher.' . $phpEx.'?action=lang_user_created');
				...
				for ($i = 0; $i < sizeof($results); $i++)
				{
					...
					$template->assign_block_vars('row', array(
						...
						'U_PACK'		=> mx_append_sid('admin_publisher.' . $phpEx . '?action=lang_user_created&amp;mode=pack&amp;pack=' . urlencode($pack_file) . '&amp;level=' . ($admin ? 'admin' : 'normal')),
						'U_KEY'			=> mx_append_sid('admin_publisher.' . $phpEx . '?action=lang_user_created&amp;mode=key&amp;pack=' . urlencode($pack_file) . '&amp;level=' . ($admin ? 'admin' : 'normal') . '&amp;key=' . urlencode($key_main). '&amp;sub=' . urlencode($key_sub)),
						)
					);
				}
		...
		if ($mode == '')
		{
			...
			while (list($pack_file, $pack_name) = @each($packs))
			{
				...
				// LANG USER CREATED AND LANG MAIN SETTINGS
				if ( (preg_match('/' . 'lang_u' . '/i', urlencode($pack_file))) || (preg_match('/' . 'lang_c' . '/i', urlencode($pack_file))) )
				{
					$l_normal = $lang['Lang_extend_level_normal'];
					$u_normal = mx_append_sid('admin_publisher.' . $phpEx . '?action=lang_user_created&amp;mode=pack&amp;pack=' . urlencode($pack_file) . '&amp;level=normal');
					$l_admin = $lang['Lang_extend_level_admin'];
					$u_admin = mx_append_sid('admin_publisher.' . $phpEx . '?action=lang_user_created&amp;mode=pack&amp;pack=' . urlencode($pack_file) . '&amp;level=admin');
				}
				else
				{
