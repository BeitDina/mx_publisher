...
class mx_module_defs
{
	...
	function get_parameters($type_row = '')
	{
		...
		$type_row['pub_mapping'] = !empty($lang['ParType_pub_mapping']) ? $lang['ParType_pub_mapping'] : "MXPub category mapping";
		$type_row['pub_quick_cat'] = !empty($lang['ParType_pub_quick_cat']) ? $lang['ParType_pub_quick_cat'] : "MXPub default category";
	...
	function display_module_parameters( $parameter_data, $block_id )
	{
		...
		{
			case 'pub_mapping':
				$this->display_edit_pub_mapping( $block_id, $parameter_data['parameter_id'], $parameter_data );
				break;

			case 'pub_quick_cat':
				$this->display_edit_pub_quick_cat( $block_id, $parameter_data['parameter_id'], $parameter_data );
				break;
		}
	}

	function display_edit_pub_quick_cat( $block_id, $parameter_id, $parameter_data )
	{
	...
	// ===================================================
	function display_edit_pub_mapping( $block_id, $parameter_id, $parameter_data )
	{
		global $template, $board_config, $db, $theme, $lang, $images, $mx_blockcp, $mx_root_path, $phpEx, $mx_table_prefix, $table_prefix;
		...
		//
		$s_action_file = $mx_blockcp->blockcp_mode == 'mx_blockcp' ? 'controller/mx_coreblocks/mx_blockcp.php' : 'admin/admin_mx_block_cp.php';
		...
		// Get varaibles
		//
		$pub_map_cat = $this->generate_jumpbox( 0, 0, '', true );
		$pub_map_dynid = get_list_formatted( 'block_list', 0, 'map_dyn_id' );
		...
		$template->assign_vars(array(
			...
			'L_MAP_PUBLISHER' 			=> $lang['Map_publisher'],
			...
			'S_MAP_CAT_LIST' 			=> $pub_map_cat,
			'S_MAP_DYN_LIST' 			=> $pub_map_dynid,
		...
		$pub_mapping_data = !empty($mx_blockcp->block_parameters['pub_mapping']['parameter_value']) ? unserialize( stripslashes( $mx_blockcp->block_parameters['pub_mapping']['parameter_value'] )) : array();

		//
		// Check that some categories exist
		//
		if ( $total_maps = count( $pub_mapping_data ) )
		{
			for( $i = 0; $i < $total_maps; $i++ )
			{
				$pub_map_cat_tmp = $this->generate_jumpbox( 0, 0, array( $pub_mapping_data[$i]['map_cat_id'] => 1 ), false );
				$pub_map_dynid_id = 'map_dyn_id_'.$i;
				$pub_map_dynid_tmp = get_list_formatted( 'block_list', $pub_mapping_data[$i]['map_dyn_id'], $pub_map_dynid_id );

				$pub_delete_url = mx_append_sid($mx_root_path . $s_action_file . $deletemode . '&amp;delete_id=' . $i . '&amp;sid=' . $userdata['session_id']);

				$template->assign_block_vars( 'map_row', array(
						'CAT_ID' => $i,
						'CAT_LIST' => $pub_map_cat_tmp,
						'DYN_ID' => $i,
						'DYN_LIST' => $pub_map_dynid_tmp,
						'DELETE' => $pub_delete_url,
						'L_DELETE' => $lang['Delete']
						) );
			}
	...
	function _do_insert( $id )
	{
		...
		if ( !MX_PANEL_DEBUG )
		{
			...
			// Get mapping
			//
			$pub_mapping_list = !empty($mx_blockcp->block_parameters['pub_mapping']['parameter_value']) ? unserialize( stripslashes( $mx_blockcp->block_parameters['pub_mapping']['parameter_value'] )) : array();

			//
			// Append mapping
			//
			$pub_mapping_list[] = array( 'map_cat_id' => intval( $_POST['map_cat_id'] ), 'map_dyn_id' => intval( $_POST['map_dyn_id'] ) );

			$pub_mapping_data = addslashes( serialize( $pub_mapping_list ));

			$sql1 = "UPDATE " . BLOCK_SYSTEM_PARAMETER_TABLE . "
			  		SET parameter_value = '" . $pub_mapping_data . "'
			  		WHERE block_id     = $block_id
			  	  		AND parameter_id = '".$mx_blockcp->block_parameters['pub_mapping']['parameter_id']."'";
	...
	function _do_update( $id )
	{
		...
		if ( !MX_PANEL_DEBUG )
		{
			...
			{
				$pub_cat_key = 'map_cat_id_' . $i;
				$pub_dyn_key = 'map_dyn_id_' . $i;

				$pub_mapping_list[$i] = array( 'map_cat_id' => intval( $_POST[$pub_cat_key] ), 'map_dyn_id' => intval( $_POST[$pub_dyn_key] ) );
			}
	...
	function _do_delete( $id )
	{
		...
		if ( !MX_PANEL_DEBUG )
		{
			...
			$pub_mapping_list_tmp = array();

			for ( $i = 0; $i < count($pub_mapping_list); $i++ )
			{
				if ( $i != intval( $_GET['delete_id'] ) )
				{
					$pub_mapping_list_tmp[] = $pub_mapping_list[$i];
				}
			}

			$pub_mapping_data = addslashes( serialize( $pub_mapping_list_tmp ));
